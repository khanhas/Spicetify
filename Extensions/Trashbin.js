  ...
  */
    ...
    if (jsonBinURL) {
      $.ajax({
        ...
        success: (data) => {
          ...
          if (oldFormat) {
            trashSongList = migrateDataToNewFormat(data['TrashSongList']);
            trashArtistList = migrateDataToNewFormat(data['TrashArtistList']);
          } else {
      ...
      trashSongList = JSON.parse(chrome.localStorage.get('TrashSongList'));
      trashArtistList = JSON.parse(chrome.localStorage.get('TrashArtistList'));
  ...
  function watchArtistPage() {
    ...
    function appendArtistTrashbin() {
      const headers = $('iframe#app-artist').contents()
      ...
      const artistUri =
        `spotify:artist:${$('iframe#app-artist').attr('data-app-uri').split(':')[3]}`;
  ...
  function storeList() {
    if (jsonBinURL) {
      ...
      chrome.localStorage.set("TrashSongList", JSON.stringify(trashSongList));
      chrome.localStorage.set("TrashArtistList", JSON.stringify(trashArtistList));
    }
  }
  ...
  function updateIconColor_Artist() {
    const buttons = $('iframe#app-artist').contents().find('.throw-artist');
  ...
  function dataIsInOldFormat(data) {
    return Array.isArray(data)
      || (!Array.isArray(data) && (!!data['TrashSongList'] || !!data['TrashArtistList']));
  }
